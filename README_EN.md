...
ICM GPIO395 INT1 rising edge
  -> CLOCK_MONOTONIC_RAW raw timestamp -> TIME_BASE frozen offset -> system_realtime host_timestamp_ns
...
- `group_ts_ns`: frame-set timestamp in `ns`; its domain is declared by `group_ts_domain`
...
- `camera_ts_ns`: camera frame timestamp in `ns`; `software_gpio`/`gpio` mode is mapped through the frozen `TIME_BASE` offset to `system_realtime`, while other trigger modes preserve the original `libsc132.so` `sc132_native` domain
- `group_ts_domain` / `camera_ts_domain` / `rtsp_ts_domain`: timestamp domains for the corresponding fields; values are `system_realtime`, `monotonic_raw`, or `sc132_native`
- `rtsp_ts_ns`: frame timestamp passed to PRRTSP; its domain is declared by `rtsp_ts_domain`
...
- `ts_ns`: `system_realtime` timestamp mapped through the frozen `TIME_BASE` offset, in `ns`
...
- Each rising edge is first timestamped with `CLOCK_MONOTONIC_RAW` before the 14-byte direct SPI read; `imu_reader_demo` maps that raw value through the startup `TIME_BASE` into per-sample `system_realtime` `host_timestamp_ns`, not a copied FIFO batch timestamp.
